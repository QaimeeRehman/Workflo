...
function OrderDetailsPage({ order, inventory }) {
  ...
  async function handleMarkReady() {
    ...
    try {
      ...
      if (!result.success) {
        toast.error(result.error || "Failed to ready order");
        return;
      }

      toast.success(`Order ready : ${result.order.order_number}`);

      router.refresh();
    } catch (error) {
      console.log("handleMarkReady", error);
      toast.error("Something went wrong while marking ready order.");
    } finally {
      setIsPending(false);
    }
  }
  ...
  async function handleCreateBill() {
    try {
      const result = await createPreOrderBill(order);
      toast.success(
        `Invoice ${result.bill.bill.invoice_number} created successfully`,
      );
      console.log(result);
      router.push(`/dashboard/billing/${result.bill.bill.token}`);
    } catch (error) {
      console.log("handleCreateBill: ", error);
      toast.error("Something went wrong while creating bill.");
    }
  }
